...
export default function PuzzleFavoriteStar({
  puzzleId,
  size = '1.72rem',
}: PuzzleFavoriteStarProps): ReactNode {
  ...
  useEffect(() => {
    ...
    window.addEventListener(PUZZLE_FAVORITES_EVENT_NAME, syncFavoriteState as EventListener);
    return () => {
      window.removeEventListener('storage', syncFavoriteState);
      window.removeEventListener(PUZZLE_FAVORITES_EVENT_NAME, syncFavoriteState as EventListener);
    };
  }, [puzzleId]);
  ...
  return (
    <button
      type="button"
      aria-label={isFavorite ? 'Remove puzzle from favorites' : 'Save puzzle to favorites'}
